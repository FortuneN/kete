...
public class isHealthyTests {
	...
	private void startBroker(int hostPort) {
		...
		broker = new GenericContainer<>(DockerImageName.parse("nats:2.10-alpine"))
			.withCommand("-js")
			.withCreateContainerCmdModifier(cmd -> cmd.getHostConfig()
				.withPortBindings(new PortBinding(Ports.Binding.bindPort(hostPort), new ExposedPort(BROKER_PORT))));
		broker.start();

		// (re)create the stream after every broker start - a fresh container has none

		await().atMost(Duration.ofSeconds(60)).pollInterval(Duration.ofSeconds(2)).until(() -> {
			try (var nc = Nats.connect("nats://127.0.0.1:" + hostPort)) {
				nc.jetStreamManagement().addStream(StreamConfiguration.builder().name("TEST_STREAM").subjects("test.subject").build());
				return true;
	...
	public void shouldReportUnhealthyDuringBrokerOutageAndResumeAfterRestart() throws Exception {
		...
		var first = awaitSuccessfulSend("evt-baseline", Duration.ofMinutes(2));
		...
		awaitSuccessfulSend("evt-resumed", Duration.ofMinutes(2));
	}
}
