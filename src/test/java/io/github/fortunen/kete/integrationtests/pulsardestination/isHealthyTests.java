...
public class isHealthyTests {
	...
	private GenericObjectPool<Destination<?>> createPool(int mappedPort) {
		...
		map.put("kind", "pulsar");
		map.put("service-url", "pulsar://127.0.0.1:" + mappedPort);
	...
	public void shouldReportUnhealthyDuringBrokerOutageAndResumeAfterRestart() throws Exception {
		...
		var first = awaitSuccessfulSend("evt-baseline", Duration.ofMinutes(5));
		...
		awaitSuccessfulSend("evt-resumed", Duration.ofMinutes(5));
	}
}
