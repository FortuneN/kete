...
Realms that exist when Keycloak starts are processed at start-up. A realm created later is registered the moment it is created (KETE listens for realm-creation events) — no restart is needed; the route realm matchers are evaluated against the new realm's name.
