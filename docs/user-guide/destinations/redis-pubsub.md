...
| `tls.verify-hostname` | `false` | `true` verifies the server certificate chain against the trust store **and** the hostname. `false` (the default, kept for backward compatibility) makes the Redis client skip certificate validation entirely — the trust store is not consulted. Set it to `true` for any Redis reached over an untrusted network. |
