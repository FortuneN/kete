...
public class TestBase {
	...
	protected Mqtt5Destination destination;
	protected Mqtt5DestinationConfig config;

	@BeforeEach
	void setUp() {
		destination = new Mqtt5Destination();
		config = new Mqtt5DestinationConfig();
	}
	...
	protected AutoCloseable createSubscriber(String topic, int qos, MessageCallback callback) throws Exception {
		...
		// Use MqttSubscription and IMqttMessageListener arrays to avoid StackOverflow
		// bug in Paho MQTT5 client's subscribe(String, int, IMqttMessageListener) method
		var subscription = new MqttSubscription(topic, qos);
		IMqttMessageListener messageListener = (t, msg) -> callback.messageArrived(t, msg);
		client.subscribe(new MqttSubscription[] { subscription }, new IMqttMessageListener[] { messageListener });
