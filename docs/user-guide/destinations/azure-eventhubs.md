...
| `destination.timeout-seconds` | _(Azure SDK default: 60)_ | Per-operation (AMQP try) timeout in seconds for sends and the start-up connection check. Applied only when set; otherwise the Azure SDK default is kept. | `30` |
